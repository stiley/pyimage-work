...
################################################################
# See this page for discussion https://gurus.pyimagesearch.com/topic/simple-contour-properties/
################################################################
...
for c in cnts:
    ...
    M = cv2.moments(c)
    # print("For this countour, M looks like this {}".format(M))
    cX = int(M["m10"] / M["m00"])
    cY = int(M["m01"] / M["m00"])

    # draw the center of the contour on the image
    cv2.circle(clone, (cX, cY), 10, (255, 0, 0), -1)
...
for (i, c) in enumerate(cnts):
    ...
    cX = int(M["m10"] / M["m00"])
    cY = int(M["m01"] / M["m00"])
    print("Centroid for contour {} is located at ({},{})".format(i + 1,cX,cY ))
...
for (i, c) in enumerate(cnts):
    ...
    print("Bunding rect for contour {} x={} y={} w={} h={})".format(i + 1, x, y, w, h))
...
for (i, c) in enumerate(cnts):
    ...
    print("Min circle for contour {} centered at point ({},{}) with a radius of {}".format(i + 1, x, y, radius))
    cv2.circle(clone, (int(x), int(y)), int(radius), (255, 0, 0), 1)
